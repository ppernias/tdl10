...
The absence of a \textit{student model} is a deliberate limitation: TDL does not aim to diagnose the student's cognitive state but rather to structure pedagogical interactions coherently. The underlying LLM provides some conversational adaptation but not the systematic tracking of a traditional ITS.
...
\begin{itemize}
    \item \textbf{Command syntax}: Defines commands such as \texttt{/start} (begin the first unit), \texttt{/next} (advance to the next unit), and \texttt{/progress} (show the current state).

    \item \textbf{State tracking mechanism}: Uses explicit markers in the format \texttt{[UNIT:\{id\}|EVENT:\{id\}]} to maintain context between conversation turns.
...
\begin{itemize}
    ...
    \item \textbf{Event sequence}: An ordered list of steps the tutor must follow.
    \item \textbf{Instructions per event}: What the tutor should do at each event.
    \item \textbf{Transition triggers}: The conditions that trigger advancement to the next event.
\end{itemize}
...
This concept is functionally analogous to Merrill's \textit{transaction shells} \cite{merrill1991}: reusable pedagogical algorithms for different content.
...
\begin{itemize}
    \item \textbf{Tutor profile}: Knowledge domain, role, style, and supported languages.
    \item \textbf{Model inheritance}: Which instructional model to use (via \texttt{extends}).
    \item \textbf{Behaviors}: Initial greetings, help responses, off-topic handling, and disclaimers.
...
TDL supports multiple content source formats: Markdown (recommended), plain text, PDF, and Word. The learning sequence references specific content sections via the \texttt{source\_section} field.
...
\begin{enumerate}
    ...
    \item The Engine interprets the message and determines the current context (unit, event).
